...
// @updateURL         https://raw.githubusercontent.com/gnadgnaoh/Shortlink/release/release/panlinker-modified.meta.js
// @downloadURL       https://raw.githubusercontent.com/gnadgnaoh/Shortlink/release/release/panlinker-modified.user.js
...
(function () {
    ...
    let base = {
        ...
        showSetting() {
            ...
            dom += `<label class="pl-setting-label"><div class="pl-label">RPC Host</div><input type="text"  placeholder="Host address, begin with http(s)://" class="pl-input listener-domain" value="${base.getValue('setting_rpc_domain')}"></label>`;
            dom += `<label class="pl-setting-label"><div class="pl-label">RPC Port</div><input type="text" placeholder="Port number. Example: 16800 for Motrix" class="pl-input listener-port" value="${base.getValue('setting_rpc_port')}"></label>`;
            dom += `<label class="pl-setting-label"><div class="pl-label">RPC Path</div><input type="text" placeholder="Default is /jsonrpc}" class="pl-input listener-path" value="${base.getValue('setting_rpc_path')}"></label>`;
            dom += `<label class="pl-setting-label"><div class="pl-label">RPC Key</div><input type="text" placeholder="Leave it blank if not have key" class="pl-input listener-token" value="${base.getValue('setting_rpc_token')}"></label>`;
            dom += `<label class="pl-setting-label"><div class="pl-label">Save location</div><input type="text" placeholder="Path to save files after download. Example: D:" class="pl-input listener-dir" value="${base.getValue('setting_rpc_dir')}"></label>`;
            ...
            dom += `</select></label>`;
            dom += `<label class="pl-setting-label"><div class="pl-label">Theme color</div> <div class="pl-color">${btn}<div></label>`;
            dom = '<div>' + dom + '</div>';
    ...
    let baidu = {
        ...
        addPageListener() {
                ...
                ins[index] = setInterval(() => {
                    ...
                    if (isIDM) {
                        ...
                        if (prog === 100) {
                            ...
                            o.item.find('.pl-progress-tip').html('Download completed and browser will popup download box！');
                        }
                    }
                }, 500);
            });
            ...
            doc.on('click', '.listener-link-aria, .listener-copy-all', (e) => {
                ...
                if (!e.target.dataset.link) {
                    ...
                    $(e.target).text('Copied successfully!').animate({opacity: '0.5'}, "slow");
                }
            });
        ...
        addButton() {
            if (!pt) return;
            let $toolWrap;
            let $button = $(`<div class="g-dropdown-button pointer pl-button"><div style="color:#fff;background: ${color};border-color:${color}" class="g-button g-button-blue"><span class="g-button-right"><em class="icon icon-download"></em><span class="text" style="width: 60px;">Download</span></span></div><div class="menu" style="width:auto;z-index:41;border-color:${color}"><div style="color:${color}" class="g-button-menu pl-button-mode" data-mode="api">API Download</div><div style="color:${color}" class="g-button-menu pl-button-mode" data-mode="aria">Aria Download</div><div style="color:${color}" class="g-button-menu pl-button-mode" data-mode="rpc">RPC Download</div><div style="color:${color}" class="g-button-menu pl-button-mode" data-mode="curl">cURL Download</div><div style="color:${color}" class="g-button-menu pl-button-mode" data-mode="bc">BC Download</div><li class="g-button-menu pl-button-mode listener-open-setting">助手设置</li>${pan.code == 200 && version < pan.version ? pan.new : ''}</div></div>`);
        ...
        addInitButton() {
            if (!pt) return;
            let $toolWrap;
            let $button = $(`<div class="g-dropdown-button pointer pl-button-init" style="opacity:.5"><div style="color:#fff;background: ${color};border-color:${color}" class="g-button g-button-blue"><span class="g-button-right"><em class="icon icon-download"></em><span class="text" style="width: 60px;">Download</span></span></div></div>`);
            if (pt === 'home') $toolWrap = $(pan.btn.home);
            if (pt === 'main') {
                $toolWrap = $(pan.btn.main);
        ...
        async getPCSLink(maxRequestTime = 1) {
            ...
            if (pt === 'home' || pt === 'main') {
                if (selectList.length === 0) {
                    return message.error('Please choose files you want to download first！');
                }
                if (fidList.length === 2) {
                    return message.error('Please open folder and then select files！');
                }
        ...
        generateDom(list) {
            ...
            if (mode === 'curl')
                content += `<div class="pl-extra"><button class="pl-btn-primary listener-copy-all" data-link="${alinkAllText}">Copy all links</button><button class="pl-btn-primary pl-btn-warning listener-open-setting" style="margin-left: 10px;">Terminal type (Currently:${terminalType[base.getValue('setting_terminal_type')]}）</button></div>`;
            return content;
        },
    ...
    let tianyi = {
        ...
        addButton() {
            if (!pt) return;
            let $toolWrap;
            let $button = $(`<div class="tianyi-button pl-button">Download<ul class="pl-dropdown-menu" style="top: 26px;"><li class="pl-dropdown-menu-item pl-button-mode" data-mode="api">API Download</li><li class="pl-dropdown-menu-item pl-button-mode" data-mode="aria" >Aria Download</li><li class="pl-dropdown-menu-item pl-button-mode" data-mode="rpc">RPC Download</li><li class="pl-dropdown-menu-item pl-button-mode" data-mode="curl">cURL Download</li><li class="pl-dropdown-menu-item pl-button-mode" data-mode="bc" >BC Download</li><li class="pl-dropdown-menu-item pl-button-mode listener-open-setting">助手设置</li>${pan.code == 200 && version < pan.version ? pan.new : ''}</ul></div>`);
            if (pt === 'home') {
                base.listenElement(pan.btn.home, () => {
                    $toolWrap = $(pan.btn.home);
                    $('.pl-button').length === 0 && $toolWrap.prepend($button);
        ...
        addInitButton() {
            if (!pt) return;
            let $toolWrap;
            let $button = $(`<div class="tianyi-button pl-button-init">Download</div>`);
            if (pt === 'home') {
                base.listenElement(pan.btn.home, () => {
                    $toolWrap = $(pan.btn.home);
        ...
        async getFileUrlByOnce(item, index, token) {
            try {
                ...
                if (res.res_code === 0) {
                    ...
                    return {
                        index,
                        downloadUrl: 'Please login first！'
                    };
    ...
    let xunlei = {
        ...
        addInitButton() {
            if (!pt) return;
            let $toolWrap;
            let $button = $(`<div class="xunlei-button pl-button-init"><i class="xlpfont xlp-download"></i><span style="font-size: 13px;margin-left: 6px;">Download</span></div>`);
            if (pt === 'home') {
                base.listenElement(pan.btn.home, () => {
                    $toolWrap = $(pan.btn.home);
                    $('.pl-button-init').length === 0 && $toolWrap.prepend($button);
        ...
        async getFileUrlByOnce(item, index, token) {
            try {
                ...
                return message.error('Please login first后刷新页面！');
            }
        },
    ...
    let yidong = {
        ...
        addButton() {
            if (!pt) return;
            let $toolWrap;
            let $button = $(`<div class="yidong-button pl-button">Download<ul class="pl-dropdown-menu" style="top: 36px;"><li class="pl-dropdown-menu-item pl-button-mode" data-mode="api">API Download</li><li class="pl-dropdown-menu-item pl-button-mode" data-mode="aria" >Aria Download</li><li class="pl-dropdown-menu-item pl-button-mode" data-mode="rpc">RPC Download</li><li class="pl-dropdown-menu-item pl-button-mode" data-mode="curl">cURL Download</li><li class="pl-dropdown-menu-item pl-button-mode" data-mode="bc" >BC Download</li><li class="pl-dropdown-menu-item pl-button-mode listener-open-setting">助手设置</li>${pan.code == 200 && version < pan.version ? pan.new : ''}</ul></div>`);
            if (pt === 'home') {
                base.listenElement(pan.btn.home, () => {
                    $toolWrap = $(pan.btn.home);
                    $('.pl-button').length === 0 && $toolWrap.prepend($button);
        ...
        addInitButton() {
            if (!pt) return;
            let $toolWrap;
            let $button = $(`<div class="yidong-button pl-button-init">Download</div>`);
            if (pt === 'home') {
                base.listenElement(pan.btn.home, () => {
                    $toolWrap = $(pan.btn.home);
                    $('.pl-button-init').length === 0 && $toolWrap.prepend($button);
